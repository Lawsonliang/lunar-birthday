---
name: lunar-birthday
description: 中国农历与新历（公历）日期互查工具、农历生日管家、节假日管家。当用户需要查询农历对应的新历日期、管理家人农历生日、检查即将到来的生日、查询全年节假日（法定假日/传统节日/节气等）时，使用此技能。触发关键词：农历、新历、公历、阴历、阳历、生日提醒、生日管家、节假日、放假、节日、节气、lunar、solar calendar、holiday。
---

# 农历生日管家 + 节假日管家 (Lunar Birthday & Holiday Manager)
...
3. **节假日管家** — 管理全年节假日（法定假日、传统节日、西方节日、节气、纪念日），检查即将到来的节日
...
- 节假日管家脚本：`scripts/holidays.py`
- 生日数据文件：`birthdays.json`（自动创建）
- 节假日数据文件：`holidays.json`（自动创建，内置2026年完整数据）
...
内置2026年全年节假日数据（法定假日、传统节日、家庭节日、网络节日、西方节日、节气、纪念日）。

### 列出所有节假日

```bash
python3 scripts/holidays.py list
```

返回所有节假日，按距离今天的天数排序，包含：
- 节日名称
- 日期和星期
- 类型（法定假日/传统节日/节气等）
- 距今天数
...
```bash
python3 scripts/holidays.py check [天数]
```

- 默认检查未来 7 天
- 可自定义天数，如 `python3 scripts/holidays.py check 30` 检查未来 30 天

### 按类型筛选节假日

```bash
python3 scripts/holidays.py filter <类型>
```

可用类型：
- `法定假日` — 劳动节、端午节、中秋节、国庆节、元旦
- `传统节日` — 七夕节、中元节、重阳节、腊八节
- `家庭节日` — 母亲节、父亲节、儿童节、教师节
- `网络节日` — 520网络情人节
- `西方节日` — 万圣节、平安夜、圣诞节
- `节气` — 立夏、小满、芒种、夏至等24节气
- `纪念日` — 青年节、护士节等
...
### 当用户说"查节日"、"最近有什么假"、"今年还有什么节日"等：

1. 运行 `python3 scripts/holidays.py check` 检查未来 7 天的节假日
2. 如果有即将到来的节日，用友好的方式提醒用户，包含：
   - 节日名称
   - 日期和星期
   - 还有几天
   - 类型和备注（如放假天数）
...
### 当用户说"列出所有节日"、"今年有哪些节日"等：

1. 运行 `python3 scripts/holidays.py list`
2. 以清晰的表格或列表形式展示所有节日信息

### 当用户说"法定假日有哪些"、"传统节日有哪些"等按类型查询时：

1. 运行 `python3 scripts/holidays.py filter <类型>`
2. 展示该类型的所有节日
